...
app.post('/responses', async (req, res) => {
  ...
  try {
    const user = await pool.query('SELECT name FROM studentusers WHERE name = $1', [username]);
...
app.get('/polls/:username/created', async (req, res) => {
  const { username } = req.params;
  try {
    const polls = await pool.query('SELECT id, question FROM polls WHERE user_name = $1', [username]);
    res.json(polls.rows);
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ error: 'Server error' });
...
app.get('/download/:pollId', async (req, res) => {
  ...
  try {
    ...
    doc.fontSize(20).text(`Poll Results: ${poll.rows[0].question}`, { align: 'center' });
    doc.moveDown();
